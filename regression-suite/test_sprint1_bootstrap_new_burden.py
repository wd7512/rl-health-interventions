"""Regression tests for sprint1_bootstrap_new_burden experiments.

Re-runs all bayesian-burden window-size configs at fixed seeds and compares
against golden JSON fixtures stored in
docs/experimental_phases/sprint1_bootstrap_new_burden/results/.
...
def new_burden_results() -> dict[str, dict]:
    """Run the bayesian burden benchmark once and return all config results."""
...
def test_new_burden_regression(
    config_name: str, new_burden_results: dict[str, dict]
) -> None:
    ...
    live_agents = new_burden_results.get(config_name, {})
