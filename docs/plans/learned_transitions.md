...
The MVP simulator (`config/rule_based.yaml`, `src/rl_health_interventions/transitions/rule_based.py`) uses a static, hand-typed probability table. The canonical design doc (`docs/design/initial_design.tex:140-178`) describes richer dynamics (continuous state from wearables, action-conditioned response, burden accumulation) that the rule-based path cannot represent without bespoke engineering per dataset.
...
- Update `docs/design/initial_design.tex` MDP decision log table — mark transition row as `Confirmed: learned (residual ridge) on 4TU#1`.
- Update `docs/plans/ROADMAP.md` to acknowledge the Phase 1 / Phase 2 boundary has been crossed (per grilling Q3 framing). Add a Phase 2 item for the NN upgrade path.
