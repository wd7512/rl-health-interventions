...
(e.g. $\eta \cdot \text{BM\_improvement} \cdot \gamma^{3k-t}$ between
...
measure delay (3 epochs), and practical deployment constraints. Whether
...
agent benchmarking. Unified data loaders for all open datasets are provided
by the data layer (see \texttt{src/rl\_health\_interventions/data/}).
...
Which burden model is most appropriate? & Linear accumulator $\max(0, burden_t + penalty)$ with threshold decay. StepCountJITAI uses bounded $[0,1]$ habituation. & Open \\
...
Sparse vs decaying delayed reward? & 3-week sparse (current). Decaying formulation ($\eta \cdot BM \cdot \gamma^{3k-t}$) may improve credit assignment. & Open \\
\bottomrule
\end{tabular}
\end{table}
