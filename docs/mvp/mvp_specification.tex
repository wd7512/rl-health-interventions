...
All agents are bandits: they do not condition on state. From the transition matrix,
...
This gap motivates the state-aware agents planned for Phase~2.
...
The mixture chain $P_{\text{random}} = \tfrac{1}{2}(P_{\text{nudge}} + P_{\text{idle}})$ gives $E[r] \approx 0.308$ per step, matching the empirical $0.301 \pm 0.024$ within one standard deviation.
