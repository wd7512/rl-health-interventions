...
All agents run on the same config (\texttt{config/mvp.yaml}) over 10 random seeds (1--10), 450 timesteps each. Results are reported as mean $\pm$ standard deviation across seeds.
...
\midrule
Thompson Sampling & 161.9 $\pm$ 15.4 & 0.360 $\pm$ 0.034 \\
Epsilon-Greedy ($\epsilon=0.1$) & 155.9 $\pm$ 17.7 & 0.346 $\pm$ 0.039 \\
UCB ($c=2.0$) & 148.5 $\pm$ 20.1 & 0.330 $\pm$ 0.045 \\
Random & 133.6 $\pm$ 14.8 & 0.297 $\pm$ 0.033 \\
\bottomrule
\end{tabular}
\caption{Results on \texttt{config/mvp.yaml} (10 seeds, 450 timesteps each). TS leads with the highest mean and lowest variance among learning agents. UCB outperforms random but shows higher variance than TS and $\epsilon$-greedy.}
\end{table}
