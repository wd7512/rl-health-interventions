"""Generate learning curve and hyperparameter charts for sprint1_bootstrap agents.
...
def _parse_dec_df(rows: list[dict[str, str]], ctx: bool = False) -> list[dict]:
    ...
    for row in rows:
        ...
        result.append(
            {**row, "epsilon_start": epsilon_start, "decay_steps": decay_steps}
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", type=str, default=None, help="Agent config filename"
    )
    parser.add_argument(
        "--hp-results",
        type=str,
    ...
    config_path = resolve_config(
        args.config, default="sprint1_bootstrap_extensions.yaml"
    )
    ...
    bounds = None
    if compute_bounds is not None:
        try:
            bounds = compute_bounds(config)
        except ValueError as e:
            logger.warning(
                "Could not compute optimal bounds: %s -- skipping optimal lines", e
            )
