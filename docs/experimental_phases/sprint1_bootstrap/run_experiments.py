"""E2E benchmark: compare all agents on the sprint1 bootstrap-transition MDP.

Loads a sprint1_bootstrap MDP config with an agents section, runs every agent variant.
"""
...
_SPRINT1_BOOTSTRAP_CONFIGS = [
    _CONFIGS_DIR / "sprint1_bootstrap.yaml",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark agents on sprint1 bootstrap-transition MDP"
    )
    parser.add_argument(
        "--seeds",
        type=_positive_int,
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Run all sprint1_bootstrap configs",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    if args.all:
        for config_path in _SPRINT1_BOOTSTRAP_CONFIGS:
            logger.info("\n=== Config: %s ===\n", config_path.name)
            _benchmark_config(
                str(config_path),
                n_seeds,
