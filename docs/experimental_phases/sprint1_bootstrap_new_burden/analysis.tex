...
\noindent The original w7d mapping has 14/11/11 values for low/medium/high
(38.9\%/30.6\%/30.6\% of the 36-count range).  The rebalanced mapping
has 14/6/16 values (38.9\%/16.7\%/44.4\%), shifting 5 counts
(13.9\% of the range) from \texttt{medium} to \texttt{high}.  These
are count-space proportions---observed burden probabilities differ
because the failure count distribution is not uniform.
...
\toprule
Agent & Baseline & w1d & w3d & w7d & w7d\_rebal & w14d & w30d \\
\midrule
...
\noindent w7d\_rebal denotes the rebalanced mapping
...
\quad Bayesian w7d\_rebal  & 0.178 & 0.651 & 0.170 \\
...
\quad Bayesian w7d\_rebal  & 0.302 & 0.530 & 0.169 \\
...
\quad Bayesian w7d\_rebal  & 0.207 & 0.594 & 0.200 \\
\quad Bayesian w30d  & 0.392 & 0.608 & 0.000 \\
\bottomrule
\end{tabular}
\end{center}

\noindent w7d\_rebal denotes the rebalanced mapping.  The rebalanced mapping
shifts the observed distribution from 18/81/0.5 to approximately 18/65/17
for the Random agent, increasing the high-burden fraction from 0.5\% to
17\%.
...
\begin{enumerate}
  ...
  \item \textbf{Default window: w7d (35 steps) with rebalanced mapping.}
    The rebalanced mapping (\texttt{w7d\_rebalanced};
    \{0--13: low, 14--19: medium, 20--35: high\})
    produces an observed burden distribution of approximately 18/65/17
    (Random agent), increasing the high-burden fraction from 0.5\% to
    17\% compared to the original proportional mapping.
...
\begin{verbatim}
# Run all bayesian burden configs including rebalanced (50 seeds)
